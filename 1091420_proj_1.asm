...
f:
	blt a0,x0 R6	  #x < 0，跳R6
	beq a0,x0, R1	  #x == 0，跳R1
	addi t0, a0, -1
	beqz t0,R5	      #x == 1，跳R5
	addi t0, t0, -10
	bltz t0, R21	  #1 < x <= 10，跳R21
	addi t0, t0, -10
	bltz t0, R23	 #10 < x <= 20，跳R23
	beqz zero, RD5	 #x > 20，跳RD5
...
R23:
	...
	lw t1, 0(x2)		#get  x
	sw a0, 0(x2)		#store f(x-3) 
	addi a0, t1, -2
	jal f		        #f(x-2)
	
	lw t1, 0(x2)		#get f(x-3)
	add a0, a0, t1		#f(x-2) + f(x-3)
